...
FileInfo* findFileById(const char* id, FileList* list);
    ...
    AlbumList* albumList)
{
    ...
    if (parser == 0) {
        ...
        XML_SetUserData(parser, static_cast<void*>(&userData));

        len = fread(buffer, sizeof(char), sizeof(buffer), fileHandle);
        while (len > 0) {
            if (XML_Parse(parser, buffer, static_cast<int>(len),
                    len < BUFFER_SIZE) == 0) {
...
{
    ExpatUserData *data = static_cast<ExpatUserData*>(userData);
    ...
    if (strcmp(name, "FileList") == 0) {
        ...
        while (atts[index] != 0) {
            if (strcmp(atts[index], "count") == 0) {
                ++index;
                data->fileList->count =
                    static_cast<unsigned int>(atoi(atts[index]));
            }
            ++index;
        }

        if (data->fileList->count > 0) {
            data->fileList->list = static_cast<FileInfo*>(
                malloc(data->fileList->count * sizeof(FileInfo)));
        ...
        if (data->albumList->count > 0) {
            data->albumList->list = static_cast<Album*>(
                malloc(data->albumList->count * sizeof(Album)));
        ...
        while (atts[index] != 0) {
            if (strcmp(atts[index], "id") == 0) {
                ...
                ++index;
                data->currentIndex = 0;
                data->currentAlbum->count =
                    static_cast<unsigned int>(atoi(atts[index]));
                if (data->currentAlbum->count > 0) {
                    data->currentAlbum->songs = static_cast<char**>(
                        malloc(data->currentAlbum->count * sizeof(char*)));
                }
...
endXml(void* userData, const char* name)
{
    ...
    if (strcmp(name, "FileList") == 0) {
        ...
        if (strcmp(name, "filePath") == 0) {
            ...
            data->currentFile->year =
                static_cast<unsigned int>(atoi(data->valueBuffer));
        } else if (strcmp(name, "frameCount") == 0) {
            data->currentFile->frameCount =
                static_cast<unsigned int>(atoi(data->valueBuffer));
        ...
        if (strcmp(name, "algorithm") == 0) {
            ...
            file = findFileById(data->valueBuffer, data->fileList);
            if (file != 0) {
...
valueXml(void* userData, const char* val, int len)
{
    ...
    unsigned int length = iLen < BUFFER_SIZE - oldSize ? iLen :
        BUFFER_SIZE - oldSize;
...
{
    *destination =
        static_cast<char*>(malloc((strlen(source) + 1) * sizeof(char)));
    strcpy(*destination, source);
    (*destination)[strlen(source)] = '\0';
}

FileInfo*
findFileById(const char* id, FileList* list)
{
    FileInfo *result = 0;
    FileInfo *curr = 0;
    unsigned int index = 0;
