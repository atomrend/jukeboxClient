...
toLowerCase(char output[256], const char* const input, int length)
{
	...
	while (input[index] && index < length) {
		output[index] = static_cast<char>(tolower(input[index]));
		++index;
	}
    output[index] = '\0';
}
...
tokenize(Token** output, const char* const input)
{
    ...
    while (index < strlen(input)) {
        if (input[index] == '"') {
            ...
            operatorIndex[result] = static_cast<int>(index);
            ++result;
        } else if (strncmp(input + index, "!=", 2) == 0) {
            operatorIndex[result] = static_cast<int>(index);
            ++result;
    ...
    *output = static_cast<Token*>(malloc(result * sizeof(Token)));
    token = *output;
    index = 0;
    while (index < result) {
        token->left = 0;
    ...
    while (index < result) {
        if (token->left > 0) {
            ...
            strncpy(token->right, input + start,
...
evaluateToken(const FileInfo* const song, const Token* const token)
{
    ...
    case 6:
        ...
        break;
    case 7:
        leftInteger = static_cast<unsigned int>(song->favorite);
        break;
    default:
        break;
    }
    ...
    if (token->left > 0 && token->left < 6) {
        ...
        // Integer comparison.
        rightInteger = static_cast<unsigned int>(atoi(token->right));
...
{
    unsigned char stack[STACK_SIZE];
    unsigned char *tokenResults = static_cast<unsigned char*>(calloc(numTokens,
...
{
    char *result = 0;
    char *vector = static_cast<char*>(calloc(fileList->count, sizeof(char)));
    ...
    *count = 0;
    ...
    if (numTokens > 0) {
        ...
        result = static_cast<char*>(malloc(*count * 17 * sizeof(char)));
